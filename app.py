...
with st.expander("Quick Dataset Preview", expanded=True):
    ...
    with left_col:
        ...
        # Build the dtype table as plain Python strings — never pass raw dtype objects
        # to st.dataframe because pyarrow cannot serialize numpy dtype objects in newer versions.
...
with st.spinner("Running the model"):
    ...
    try:
        ...
        # Detect and encode text/categorical columns.
        # We check column dtypes individually rather than using select_dtypes
        # because pandas 3 changed string storage (no longer "object"),
        # and relying on select_dtypes("object") misses those columns.
